...
def remove_kth_lest_node(head: Optional[List_Node], k: int) -> Optional[List_Node]:
    ...
    for _ in range(k):
        assert leader.next is not None
        leader = leader.next
        if not leader:
            return head
    ...
    while leader.next:
        leader = leader.next
        trailer = trailer.next  # type: ignore

    assert trailer.next is not None  # explicitly inform MyPy
